...
/*   Updated: 2018/01/30 18:41:38 by ftreand     ###    #+. /#+    ###.fr     */
/*                                                         /                  */
/*                                                        /                   */
/* ************************************************************************** */
...
char	*ft_recup_full_path(char **av, t_dir *dirent, t_flag flag)
{
	char *tmp;
	char *tmp1;
	int i;
...
void	ft_recup_stat(t_ls *ls, DIR *dir, char **av, t_flag flag)
{
	t_dir	*dirent;
	t_st	stats;
	t_gr	*group;
	...
	{
		path = ft_recup_full_path(av, dirent, flag);
		stat(path, &stats);
		...
		ft_putstr(ls->time);
		SP;
		ft_strcpy(ls->d_name, dirent->d_name);
